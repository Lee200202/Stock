...
目前交付：**2026-09-25-trend-span-v68**。進場一律取當日最低、出場一律取當日最高（持有聲明、轉觀望不碰、逾期出場也一樣，撤回 v61 的收盤基準）；個股面板走勢圖直接標持有期間、出場原因改灰字、區間改成第二顆膠囊。請依 [v68 部署與驗證](docs/0925-v68/部署與驗證.md) 在原 Apps Script 專案替換 7 檔並部署，部署後以 `day-edit-sync` mode=performance 重算持股與歷史績效。Apps Script 原始碼只在本機資料夾。

目前交付：**2026-09-25-ops-kpanel-v67**。後台自動化監控（休市一致、觸發器累計時間、寄信額度估算、前一交易日寄送結果、逐日編輯同步失敗原因）與個股面板改版（週期膠囊分段、均線圖例、滾輪捲頁、K 線進出場標記、進場以來走勢小圖、卡片進場日＝成交日）。請依 [v67 部署與驗證](docs/0925-v67/部署與驗證.md) 在原 Apps Script 專案替換 15 檔並部署新版本；Apps Script 原始碼與部署包只在本機資料夾，GitHub 推送不會自動部署網站。本次 GitHub 只更新 `scripts/day_sync.py`（失敗原因寫進 Actions）與 7 個 workflow（ubuntu-24.04、checkout@v5、setup-python@v6）。
...
目前交付：**2026-09-24-no-show-v56**。預告停播的取稿、每日判讀、後台備援與狀態畫面已共用判定；請依 [v56 停播處理部署與驗證](docs/0924-v56/部署與驗證.md) 更新原 Apps Script 網頁應用程式。GitHub 推送不會自動部署網站。
...
2026/09/26 的技術說明 v70 改成連續閱讀文章與可連動的資料關聯／流程全貌圖，校正持股取價、取稿派工和休市說明。完整 Apps Script 專案封包在 `release/zhangzhen-full-v70.zip`，實際須替換的六個檔案及驗證步驟見 `docs/0926-v70/部署與驗證.md`。此儲存庫的 `apps-script/` 不上傳，GitHub 推送不會自動部署 GAS。
