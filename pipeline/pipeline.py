...
_META_CLAUSE = re.compile(r"歷史回顧|列為觀望|列入觀望|改列|日期未明|依原文語氣|依上下文判|原判|原為買入|原為賣出|不列入當日"
                          r"|非影片當日|非當日執行|依規則|歷史買進|歷史賣出"
                          # 「為什麼歸到這一類」也是內部流程，不是講者說的話。2026/09/15 的台積電寫成
...
def _drop_meta_clauses(t: str) -> str:
    ...
    for clause, punct in parts:
        if not clause.strip():
            continue
        if not _META_CLAUSE.search(clause):
            kept.append([clause, punct])
            continue
        # 丟掉的那一句原本結束了一個句子時，把句號讓給前一句留下來的子句，
        # 否則「……的影響，台積電季線下方本為正常買點」會黏成一句。
        if punct == "。" and kept:
            kept[-1][1] = "。"
    return "".join(c + p for c, p in kept)
...
def clean_meta_reason(text: str) -> str:
    ...
    # 不在括號裡的判斷子句也要拿掉（例如「，屬於歷史回顧，列為觀望注意」），
    # 以及「回顧過去在」這種把事實包成回顧的開頭。
    # 2026/09/10 的說明寫成「回顧過去在大漲時賣出索羅門，屬於歷史回顧，列為觀望注意。」，
    # 讀者要看的只有「大漲時賣出索羅門」。
...
# 目前立場只收中文。模型偶爾回英文（2026/09/10 會員持股出現兩列「long」），網站會照字面顯示。
_STANCE_EN = {"long": "續抱", "hold": "續抱", "holding": "持有", "keep": "續抱", "bullish": "續抱",
              "add": "加碼", "buy": "加碼", "accumulate": "加碼",
              "reduce": "減碼", "trim": "減碼", "sell": "減碼", "short": "減碼", "bearish": "減碼"}


def stance_zh(value) -> str:
    """把目前立場換成中文；空白回「未說明」，看不懂的英文回「持有」（它就在會員持股裡）。"""
    v = str(value or "").strip()
    if not v or v in BLANK_:
        return "未說明"
    if re.fullmatch(r"[A-Za-z][A-Za-z _-]*", v):
        return _STANCE_EN.get(v.lower().replace("_", " ").strip(), "持有")
    return v
...
def resolve_signals(signals: dict, transcript: str = "") -> dict:
    ...
    for key in ("buy", "sell", "watch_avoid", "watch_watch", "holdings", "history"):
        ...
        for r in signals.get(key, []):
            ...
            # 模型有時把聽到的原字改寫成別家公司：原文是「加折」，name 卻寫成「家登」、別稱留著加折，
            # 代號比對照字面就對到 3680 家登（2026/09/10）。管理者確認過的原字只要出現在
            # 這一筆的原始語音名稱或別稱裡，就以原字比對，模型填的代號也不採用。
            if raw not in CONFIRMED_NAMES:
                heard_names = [str(a).strip() for a in [r.get("原始語音名稱")] + list(r.get("aliases") or [])
                               if isinstance(a, str)]
                confirmed = next((a for a in heard_names if a in CONFIRMED_NAMES), "")
                if confirmed:
...
# 2026/09/15 管理者要求：標題不加「張震：」前綴（信件標題卡已經標明節目）。
TITLE_PREFIX = ''
...
def _price_near_name(price, quotes, names, span=25):
    ...
    def spots(s, k):
        """名稱在句子裡的位置。字面找不到時認同音字：「597以上要賣一次國具」的國具就是國巨（2026/09/10）。"""
        found = [m.start() for m in re.finditer(re.escape(k), s)]
        if found or not _has_cjk(k):
            return found
        pk = _npin(k)
        return [i for i in range(len(s) - len(k) + 1) if _npin(s[i:i + len(k)]) == pk]

    def near(num):
        for s in sentences:
            for k in keys:
                for at in spots(s, k):
                    if num in s[max(0, at - span):at + len(k) + span]:
                        return True
        return False
...
def validate_evidence(signals, transcript, date_str, after_codes=False):
    ...
    for cat in SIGNAL_CATEGORIES + ("history",):
        ...
        for row in signals.get(cat, []):
            ...
            named = any(len(_ev_norm(n)) >= 2 and _ev_norm(n) in evidence
                        for n in [name, heard] + aliases if n)
            # 引用裡寫的是同音字（漢唐→漢糖、漢堂），或直接念代號（「5289 移頂」）也算有指名。
            # 2026/09/11 宜鼎、漢唐兩檔真的有講到，就因為字面對不上被隔離，網站上少了兩檔。
...
#   加折／加哲／加澤 → 嘉澤（3533）：2026/09/10「8月25號1580以下買加折」「聖輝、新代、加折還有木德」，
#   模型自己把名稱改寫成「家登」，代號比對照字面對到 3680 家登。管理者確認是嘉澤。
#   出金城／初清程 → 勤誠（8210）：2026/09/10「ETF昨天來初清程…出金城…他還剩下95張，你就等他賣完
#   這支股票就漲」，是語音把「出清勤誠」聽成這兩種寫法；隔天講明「秦成900塊錢以下才可以買」。
#   原文只剩「秦」「程」「城」單字時，程式認不出是哪一檔，整檔就漏掉了。管理者確認是勤誠。
CONFIRMED_NAMES = {'普威': ('4966', '譜瑞-KY'), '普位': ('4966', '譜瑞-KY'), '譜位': ('4966', '譜瑞-KY'),
                   '普銳': ('4966', '譜瑞-KY'), '普瑞': ('4966', '譜瑞-KY'),
                   '加折': ('3533', '嘉澤'), '加哲': ('3533', '嘉澤'), '加澤': ('3533', '嘉澤'), '家澤': ('3533', '嘉澤'),
                   '茂聯': ('3665', '貿聯-KY'),
    ...
    '宏準': ('2354','鴻準'), '弘準': ('2354','鴻準'), '紅準': ('2354','鴻準'), '弘海': ('2317','鴻海'),
    ...
    '利望': ('3529','力旺'), '金星科': ('6533','晶心科'), '精星科': ('6533','晶心科'), '金新科': ('6533','晶心科'),
    '蹲態': ('3545','敦泰'), '漢糖': ('2404','漢唐'), '漢堂': ('2404','漢唐'), '秦城': ('8210','勤誠'),
    # 2026/09/23：「那記憶體，我會注意，昨天已經回補缺口，中秋節過後，準備要季線翻陽的立即電」
    # ——立即電＝力積電（6770），當天早上的會員簡訊買的就是它，理由也是季線將翻揚。
    # 模型把它寫成華邦電（2344）：同樣是記憶體，說明重點整段正確，公司卻換了一家。
    # 同一天「城成」是等 00981A 賣完、破 900 的勤誠（既有的出金城／初清程／秦城是同一檔）。
    '立即電': ('6770','力積電'), '立基電': ('6770','力積電'), '力基電': ('6770','力積電'),
    '城成': ('8210','勤誠'), '勤城': ('8210','勤誠'), '晶成': ('8210','勤誠'),
    '玉金光': ('3406','玉晶光'), '環球金': ('6488','環球晶'), '國具': ('2327','國巨*'),
    '聖輝': ('5536','聖暉*'), '聖暉': ('5536','聖暉*'), '有達': ('2409','友達'),
...
# 偏多、但沒有明講買點的說法（2026/09/14）。
# 先前 watch_tone 只認「看好、會漲、買點、等某價再買」這一類，其餘一律歸觀望不碰，
# 於是語氣明明偏多的兩檔被放進「語氣偏空，暫不進場」那張表：
#   裕隆「淨值高達60.65，資產價值非常高，具備長線價值，若拉回回補缺口可持續注意」
#   鴻準「外資持續買進，是相對穩健且適合資金較少者切入的標的」
# 這些是講者對這一檔的正面評價，應歸觀望注意。
# 但這一類說法比明講買點弱，所以只在「整段沒有任何負面線索」時才採用；
# 正負並存（例如「資產價值高，但追高容易套牢」）照舊歸觀望不碰。
_SOFT_POSITIVE = re.compile(
    r'(?:資產|長線|長期|投資|轉機)價值|具(?:備|有).{0,8}價值|價值(?:非常|很|相當)?高|淨值(?:非常|很|相當)?高'
    r'|(?:可以?|值得|持續|繼續)(?:注意|留意|關注|追蹤)'
    r'|穩健|適合.{0,12}(?:切入|進場|布局|佈局|買進|參與|投資)'
...
_SOFT_NEGATED = re.compile(
    ...
    r'(?:穩健|適合|有?價值|值得|成長|便宜|低估|受惠|利多|潛力|注意|留意|關注|追蹤|看多|偏多|轉強|走強|強勢)')
# 任何一個出現，就不採用上面那些較弱的正面說法。
...
#
# 他講 ETF 的時候，ETF 從來不是要你買的標的，是「正在賣的那一方」：
#   「晶成你一定要等900以下，還要等00981A賣完，你才可以進場」
#   「你在1，啊你在ETF賣完那一天，9月17號ETF出清，9月18號我馬上跟你講」
# 那是別檔的進場條件，不是對這支 ETF 的看法。
#
# 先前提示語寫「ETF 00981A 也收錄」，於是它每天佔著觀望表一列；
# 2026/09/23 還因為說明裡有「等待其賣完後才能評估個股進場時機」，
# 被語氣核對從觀望不碰改成觀望注意——等於把別人的買點記到 ETF 頭上。
# 現在一律移到 ignored：稽核仍留得住，網站與信件不再出現。
...
def normalize_watch_tones(signals):
    ...
    signals.update(out)
    if dropped:
        signals['ignored']=list(signals.get('ignored') or [])+dropped
    return signals
...
# ------------------------------------------------------------------ #
# ③ 分析師操作邏輯與教學重點：至少三點（管理者要求，2026/09/13；v22 前是第⑤章）
#
...
def ensure_article_minimums(signals, transcript, date_str):
    ...
    unique={}
    for row in market:
        if not isinstance(row,dict) or not row.get('_evidence_verified') or not str(row.get('text') or '').strip():continue
    ...
    gaps=signals.setdefault('_repair_gaps',[])
    if _QUOTA_STOP.get('daily') or budget_left()<300:
        gaps.append('盤勢內容偏短：時間或配額不足，未合併補問')
        return signals
    if not GEMINI_KEYS:
...
def audit_context_json(transcript, signals, date_str, editorial_retry=True):
    ...
    recover_context(signals, transcript)
    signals = exclude_past_recommendations(signals, transcript)
    ...
    if review_gaps:
        ...
        if repaired is not None:
            ...
            repaired = exclude_past_recommendations(repaired, transcript)
...
def normalize_price_fields(signals):
    """先做不依賴行情的價位角色檢查，避免把跌幅寫進正式股價欄。"""
    for cat in SIGNAL_CATEGORIES:
        for row in signals.get(cat, []):
...
def _watch_bias(row, view=''):
    """
    觀望方向只看講者「現在」對這一檔的看法，不看它過去是買還是賣。

    先前的規則是「原為賣出→觀望不碰、原為買入→觀望注意」。那是拿過去的動作猜現在的態度：
    大漲時賣掉的股票，他現在可能照樣看好，也可能根本沒再提。管理者規則（2026/09/11）：
    移除這個判斷，方向一律由現況看法決定——模型填的 watch_bias 優先，
    沒填就依現況看法那段話的語氣。
    """
...
def _current_view(row, segments):
    """
    這一檔在逐字稿裡「現在」的看多、看空或技術說明。回傳 (說明, 原句)；沒有就回 ('', [])。

    說明是模型填的 view；原句由 view_refs（來源編號）或 view_evidence 還原，
    而且至少一句要真的提到這一檔（名稱、聽到的原字或別稱），否則不算數——
    模型可能把別檔的看法搬過來。
    """
    ...
    simple = lambda s: re.sub(r'(?:-?KY|[＊*])$', '', _ev_norm(s), flags=re.I)
    names = [simple(n) for n in [row.get('name'), row.get('原始語音名稱')] + list(row.get('aliases') or [])
             if isinstance(n, str)]
...
# 模型把過去的買賣直接塞進觀望類時，理由常這樣寫。
_RETRO_MARK = re.compile(r'歷史回顧|回顧過去|日期未明|過去(?:曾經?)?(?:賣|買)|先前(?:已)?(?:賣|買)|前幾天(?:已)?(?:賣|買)'
                         r'|已(?:將|經|把).{0,8}賣掉')
# 只被拿來說明盤勢的股票，理由常這樣寫（「節目中回顧其這三個月停在這邊沒有動的走勢情況」）。
...
def _has_directive(text) -> bool:
    """這段說明裡有沒有講者對這一檔的指示：要買、要等、注意、不要碰、會漲會跌……"""
    t = clean_meta_reason(str(text or ''))
    return any(k in t for k in CANDIDATE_HINTS + NEG_HINTS + POS_HINTS + _EXTRA_DIRECTIVES)


def demote_watch_retrospectives(signals):
    """
    模型直接放進觀望類、理由卻在講過去的買賣的，退回回顧，交給 history_to_watch 決定。

    2026/09/11：「回顧過去在大漲時賣出索羅門，屬於歷史回顧，列為觀望注意。」
    2026/09/10：「過去賣出華城在775塊。」「過去大漲三天時已將利基電賣掉。」
    這幾筆都是模型自己塞進觀望類的，繞過了「要有現況看法才列」的檢查。
    理由裡另外還有講者現在的指示（例如「900以下是買點」）的，那段指示就是現況看法，
    帶著走，history_to_watch 會把它列回來；只有過去的買賣、沒有指示的，才會就此不列。
    """
    moved = []
    for cat in ('watch_avoid', 'watch_watch'):
        keep = []
        for r in signals.get(cat, []) or []:
            if isinstance(r, dict) and not r.get('view') and _RETRO_MARK.search(str(r.get('reason') or '')):
                r['_原分類'] = r.get('_原分類') or cat
                r.setdefault('watch_bias', cat)
                if _has_directive(r.get('reason')):
                    r['view'] = clean_meta_reason(r.get('reason'))
                    r['view_evidence'] = list(r.get('evidence') or [])
                signals.setdefault('history', []).append(r)
                moved.append(str(r.get('name') or ''))
            else:
                keep.append(r)
        signals[cat] = keep
    if moved:
        print(f"  觀望類裡的回顧退回確認現況看法：{'、'.join(moved)}")
    return signals
...
def history_to_watch(signals, date_str, ss=None, transcript=''):
    ...
    為什麼
    ------
    history 從來不寫進試算表。先前的處理是全部改列觀望、方向照「原為買入／賣出」猜，
    於是「大漲時賣出索羅門」這種只講過去、之後沒再提的，也被當成觀望注意列出來，
    說明欄還寫著「屬於歷史回顧，列為觀望注意」。管理者規則（2026/09/11）：
    過去的買賣本身不是現況；要到逐字稿其他地方找講者對這一檔現在的看多、看空或技術說明，
    有才列，說明寫事實加上那段看法；沒有就不列。

    落點
    ----
    方向見 _watch_bias（只看現況看法）。日期記在影片當天——那是他講這番話的日子；
    不當成當日買賣，不開持有回合、不計入績效事件。

    不列的情況
    ----------
    原文沒有這一檔的現況看法。
    同一天這一檔已經有別的紀錄（買賣、持股、觀望）：以那一筆為準，不重複列。
    會員簡訊當天仍持有或買進、而看法偏空：不列觀望不碰。觀望不碰會讓持股追蹤把那一檔平倉，
    ...
    for r in rows:
        ...
        # 說明只寫原文事實：過去做了什麼，加上他現在怎麼看。判斷依據不寫進去。
        r['reason'] = _merge_text(fact, view) if fact else view
        r['evidence'] = list(dict.fromkeys(list(r.get('evidence') or []) + quotes))
        signals.setdefault(bias, []).append(r)
        present |= keys
        moved.append(f'{name} → {label}：{r["reason"]}')
        note_decision('日期未明', f'依現況看法列{label}', name, view)
    signals['history'] = []
    for line in moved:
        print(f'  日期未明　{line}')
    for line in skipped:
    ...
    if moved:
        print(f'日期未明：{len(moved)} 筆依現況看法列入觀望，不進當日買賣、不計入績效事件。')
    return signals
...
def build_article(v2: str, signals: dict, date_str: str) -> str:
    ...
    if os.environ.get("GEMINI_ARTICLE_ENABLED", "false").lower() != "true":
        print("每日整理：使用最終JSON資料產生文章（標題＋四章），不另呼叫模型")
...
def signal_roster(signals, limit=40) -> str:
    """
    每一類收了哪幾檔，一行印出來。
...
def reconcile_with_prior(signals, prior, date_str, transcript=''):
    ...
    past = pool(('history',))
    # 只有經本機對本次原文確認的舊推薦，才可撤掉前一版觀望；一般ignored仍保留安全網。
    recaps={key for r in signals.get('ignored',[]) if r.get('_past_recommendation_only_verified') for key in _identity_keys(r)}
    ...
    for rows in groups.values():
        ...
        if now:
            ...
            continue
        if keys & recaps:
            accepted.append(f"{name}（前一版{'、'.join(before)}）：本輪確認只有舊推薦回顧，不公開、不沿用")
            continue
...
def _finish_transcript_refresh(ss, vid, date_str, raw, affected, on_progress=None, on_step=None):
    ...
    # 刷新網站的每一步都在 Apps Script 端執行，其中有幾步會刪列（例如撰稿前代號補齊會刪掉判成非個股的列）。
    # 2026/09/10 寫入 5 筆操作紀錄之後，網站「翻到某一天」卻一筆都沒有，而日誌裡每一步都是成功。
    # 每一步前後各數一次這一天的逐字稿紀錄，少了哪幾筆、是哪一步之後少的，印出來並記進判定歷程。
...
def classify_sms_row(state: str, detail: str, version: str, has_date: bool,
    ...
    # 若狀態明確標記為待解析（含文章已修訂、配額續跑等），一律視為待解析列，必須由 AI 重新判定
    if "待解析" in st or "已修訂" in st:
