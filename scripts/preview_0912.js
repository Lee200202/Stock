...
const root=path.resolve(__dirname,'..'),out=path.join(root,process.env.PREVIEW_OUTPUT || 'docs/0912/preview');fs.mkdirSync(out,{recursive:true});
...
(async()=>{
 ...
 for(const [label,width,height] of [['desktop',1440,1000],['mobile',390,844]]){
  ...
  assert.equal(await page.locator('#mailContent table').count(),3);
