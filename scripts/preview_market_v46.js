...
const root=path.resolve(__dirname,'..'),out=path.join(root,process.env.PREVIEW_OUTPUT || 'docs/0919-v46/preview');fs.mkdirSync(out,{recursive:true});
...
function previewRunner(success,failure){return new Proxy({}, {get:function(_,name){
 ...
 return function(){var args=Array.from(arguments);window.__previewApiCalls.push(String(name));
  ...
  else if(name==='apiGetMarketOverview'&&window.__emptyMarket)value={cards:[],sectors:[],warnings:['cache'],at:'本機空資料測試'};
  else if(name==='apiGetMarketOverview')value={at:'2026/09/19 12:00',cards:[{key:'taiex',label:'加權指數',value:47180.75,change:892.75,percent:1.93,time:'2026/09/18',source:'證交所收盤（測試資料）',unit:'點',candles:Array.from({length:60},function(_,i){var d=new Date(Date.UTC(2026,6,1+i)),c=42000+i*75+Math.sin(i)*500;return [d.toISOString().slice(0,10).replace(/-/g,'/'),c-30,c+180,c-160,c,100000];}),line:[{time:'09/17',value:46288},{time:'09/18',value:47180.75}]},{key:'usdtwd',label:'美元／新台幣',value:31.23,change:.1,percent:.32,time:'2026/09/18',source:'Yahoo 日資料（測試資料）',unit:'台幣／美元',line:[]}],sectors:{date:'2026/09/18',basis:'普通股成交金額占比',total:100000000000,rows:[{name:'電子工業',percent:80},{name:'數位雲端服務業測試長名稱',percent:20}]},warnings:[]};
  else if(name==='apiAdminDaySyncState')value={ok:true,state:{date:'2026/09/17',index:1,step:'同步郵件內容',status:'處理中',updatedMs:1},health:{}};
...
(async()=>{
 ...
 for(const [label,width,height] of [['desktop',1440,1000],['mobile',390,844],['narrow',320,740]]){
  ...
  for(const tab of ['overview','market','tracker','perf','subscribe','sms','tx']){
    await page.locator('[data-tab="'+tab+'"]').click();
    await page.waitForTimeout(30);
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),label+' '+tab+' overflow');
    if(tab==='market'){assert.equal(await page.locator('.market-card').count(),2);assert.equal(await page.locator('.market-open').count(),2);await page.screenshot({path:path.join(out,label+'-market.png'),fullPage:true});}
  }
  assert.deepEqual(errors,[]);results.push({label,dimensions,errors});await page.close();
 }
 for(const width of [320,1440]){const p=await browser.newPage({viewport:{width,height:850},reducedMotion:'reduce'});await p.route('https://**/*',r=>r.abort());await p.setContent(html.replace('window.__previewApiCalls=[];','window.__previewApiCalls=[];window.__emptyMarket=true;'),{waitUntil:'domcontentloaded'});await p.locator('[data-tab=market]').click();await p.waitForSelector('.market-empty');const box=await p.locator('.market-empty').boundingBox();assert(box.width>width*.8,'空狀態不可縮成一字欄');assert(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await p.screenshot({path:path.join(out,'market-empty-'+width+'.png'),fullPage:true});results.push({label:'market-empty-'+width,width:box.width});await p.close();}
 ...
 await adminPage.evaluate(()=>{document.getElementById('mChainStateBtn').click();});await adminPage.waitForTimeout(80);assert((await adminPage.locator('#mNoteBar').textContent()).includes('重算持股追蹤'));assert.equal(await adminPage.locator('#mchain .on').textContent(),'重算持股追蹤');
