...
//   一、祥碩：會員持股卻不在「目前持有」。
//   二、逾 10 個交易日未再提及的自動出場：出場價取「每天講了什麼」最後一次提及日的收盤。
...
assert.equal(Number(row['出場價']),closeOn(80,'2026/08/10'),'出場價＝8/10 收盤');
let json=JSON.parse(row['回合JSON']);
assert.equal(json[json.length-1].c,'2026/08/10');assert.equal(json[json.length-1].x,closeOn(80,'2026/08/10'));
assert(String(row['進場價來源']).includes('2026/08/10'),'來源欄寫明取哪一天收盤');
...
assert.equal(Number(row['出場價']),closeOn(80,'2026/08/07'),'週六提及 → 取 8/07 收盤，而不是 8/10');

// ---- 既有規則不變 ----
// 觀望不碰之後沒有下文（當天也沒有持股聲明）：照舊視為出場，取當日收盤
...
assert.equal(Number(row['出場價']),closeOn(2000,'2026/08/12'));
...
assert.equal(Number(row['進場價']),Number(cost));
...
row=r.byCode('5269');
assert(String(row['進場價來源']).startsWith('首次明講會員持有；2026/07/09 提到成本 '+c5269+'，落在持有聲明當日的成交區間'),row['進場價來源']);
...
assert.equal(Number(row['進場價']),close0805,'一年內從未成交的明講價不採用');
...
row=r.byCode('6666');
assert.equal(Number(row['進場價']),650);

console.log('PASS: same-day holding vs avoid, unconfirmed early holding no longer deletes later holdings, stale exit at last-mention close (on or before), unchanged avoid/unconfirmed rules.');
